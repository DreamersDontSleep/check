			...
			<el-form ref="estateForm" :inline="true" :model="estateForm" label-width="180px">
				...
				<el-form-item label="审核员:" style="width: 80%;">
					<template>
						...
						<el-select v-else v-model="estateForm.checker" filterable placeholder="请选择" style="width: 331px;">
				...
				<el-form-item label="文件上传" class="fl" style="display: block;">
					<el-upload ref="upload" :action="UploadUrl ()" :on-preview="handlePreview" :on-remove="handleRemove"
					 :before-remove="beforeRemove" :auto-upload="false" :on-change="handleChange" class="upload-demo" :limit="1"
					 :on-exceed="handleExceed" :file-list="fileList1" accept=".pdf" multiple v-if="lookOrEdit">
...
<script>
	...
	export default {
		...
		created() {
			...
			this.fileList1[0].name = this.estateForm.realName + '.pdf'
			this.fileList1[0].url = fileUrl
			if (this.upFileURI != "") {
		...
		methods: {
			...
			downloadWord() {
				...
				console.log($uri.substring(0,index+1))
				let para = {
